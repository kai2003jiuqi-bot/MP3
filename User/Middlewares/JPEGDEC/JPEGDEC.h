/* 来源: bitbank2's JPEGDEC 库 */
/* 功能: 支持 Baseline + Progressive JPEG 解码 */
/* 注意: 本文件为第三方库头文件, 请勿修改库逻辑 */
...
#ifndef __JPEGDEC__
#define __JPEGDEC__
#if defined( __MACH__ ) || defined( __LINUX__ ) || defined( __MCUXPRESSO ) || defined( ESP_PLATFORM ) || defined(_WIN64) || defined(STM32F405xx)
#include <stdlib.h>
#include <string.h>
#include <stdint.h>
#include <stdio.h>
...
#endif
// ARM Compiler 5 (AC5) doesn't have __builtin_bswap16/32
#if !defined(__GNUC__) && !defined(__clang__) && !defined(_MSC_VER)
#define __builtin_bswap16(x) ((uint16_t)(((x) << 8) | ((x) >> 8)))
#define __builtin_bswap32(x) (((x) << 24) | (((x) & 0xFF00UL) << 8) | (((x) & 0xFF0000UL) >> 8) | ((x) >> 24))
#endif
// Cortex-M4/M7 allow unaligned access to SRAM
#if defined(HAL_ESP32_HAL_H_) || defined(TEENSYDUINO) || defined(ARM_MATH_CM4) || defined(ARM_MATH_CM7) || defined (__x86_64__) || defined(TEENSYDUINO) || defined(STM32F405xx)
...
// Alignment macro: ARMCC uses __align(n), others use __attribute__((aligned(n)))
#ifdef __ARMCC_VERSION
#define JPEG_ALIGNED(x) __align(x)
#else
#define JPEG_ALIGNED(x) __attribute__((aligned(x)))
#endif
